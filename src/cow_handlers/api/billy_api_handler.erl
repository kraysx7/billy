...
init(Req0, OptsMap) ->
    ...
    {ok, Body} = case ApiMethod of
		     create_payment -> create_payment(Req0);
		     create_transaction -> create_transaction(Req0);
		     process_transaction -> process_transaction(Req0);
		     currency_rate -> currency_rate(Req0);
		     _ -> unknown_method()
		 end,
...
create_payment(Req0) ->
    ...
    %% На основе метода читаем из POST данных доп. параметры
    MethodDirty = proplists:get_value(<<"method">>, ReqParamsKV),
    PostData = case MethodDirty of
		   <<"qiwi">> -> lists:append(PostData0, 
					      [
					       {phone, proplists:get_value(<<"phone">>, ReqParamsKV)}
					      ]);
		   <<"qiwicards">> -> lists:append(PostData0, 
						   [
						    {phone, proplists:get_value(<<"phone">>, ReqParamsKV)}
						   ]);
		   <<"robokassa">> -> PostData0;
	...
	{ok, NormPostData} ->
	    ...
	    case billy_mysql:exec_prepared_stmt(#{stmt => get_merchant_stmt, params => [MerchantId]}) of
		{ok, [MerchantUserProplist]} ->
		    ...
		    %% Считаем сигнатуру сигнатуру из параметров
		    BillySignParams = #{bill_id => BillId, amount => Amount, ccy => Ccy, merchant_secret_key => MerchantSecretKey},
			...
			QuerySignature == CountedSignature ->
			    ...
			    SystemParamsJson = base64:decode(SystemParamsBase64),
			    SystemParamsMap = jiffy:decode(SystemParamsJson, [return_maps]),
			    
			    %% Декодировать base64 антифрод параметры из запроса
			    AntiFraudParamsJson = base64:decode(AntiFraudParamsBase64),
			    AntiFraudParamsMap = jiffy:decode(AntiFraudParamsJson, [return_maps]),
			    
			    %% Получить код валюты на основе 3х буквенного кода (iso4107)
			    CcyNumber = billy_commons:get_currency_number(#{currency_alpha => Ccy}),
			    
			    %% Считаем сумму платежа в валюте обеспечения
			    {ok, CcyRateMap} = billy_cbr_rate:get_rate(#{currency_alpha => Ccy}),
